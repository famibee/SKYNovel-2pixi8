...
import {CmnLib, getDateStr, uint, type IEvtMng, argChk_Boolean, argChk_Num, addStyle, argChk_Color} from './CmnLib';
...
//		this.scrItr.getDesignInfo(hArg);	// 必ず[':id'] を設定すること
		...
		const {wgsl = LayerMng.#wgslRuleTrans} = hArg;
		this.#spTransFore.filters = [new Filter({
			glProgram	: GlProgram.from({
				vertex	: vert ?? LayerMng.#glslRuleTransVert,
				fragment: frag ?? LayerMng.#glslRuleTransFrag,
			}),
...
//		s.filters = [...s.filters, Layer.bldFilters(hArg)];
	...
	//MARK: フィルター個別切替
		...
		a[i]!.enabled = argChk_Boolean(hArg, 'enabled', true);
	}
